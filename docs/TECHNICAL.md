...
2. **No Prototype Pollution**: Uses `Object.keys()` and bracket notation, avoiding `__proto__`
3. **JSON Safety**: `JSON.parse()` is safe from prototype pollution (creates plain objects)
...
1. **Immutable by default**: Returns new values instead of mutating input
...
5. **Single responsibility**: `coerce()` handles all logic
